...
public class DescendingComparator implements Comparator<Department> {
    @Override
    public int compare(Department o1, Department o2) {
        int result = 0;
        final int lim = Math.min(o1.getSubdivisions().size(), o2.getSubdivisions().size());
        for (int i = 0; i < lim; i++) {
            final int subdivisionResult = o2.getSubdivisions().get(i).compareTo(o1.getSubdivisions().get(i));
